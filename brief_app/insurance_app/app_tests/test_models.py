...
class PredictionHistoryModelTest(TestCase):
    ...
    def test_predictionhistory_bmi_calculation_various_cases(self):
        # Arrange
        user = UserProfile.objects.create_user(
            username="bmi_ph_user", password="testpass123"
        )
        # Act & Assert
...
class AvailabilityModelTest(TestCase):
    def test_availability_creation(self):
        availability = Availability.objects.create(
            date=date(2025, 5, 13), time_slots=["09:00", "14:00", "16:00"]
        )
...
class AppointmentModelTest(TestCase):
    def test_appointment_creation(self):
        ...
        appointment = Appointment.objects.create(
            user=user, reason="Consultation", date="2025-05-13", time="10:00"
        )
