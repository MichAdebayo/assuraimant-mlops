...
class HealthAdvicesView(TemplateView):
    ...
    template_name = (
        "insurance_app/health_advices.html"  # Assur'Cares Section View Template
    )
...
def contact_view(request):
    ...
    if request.method == "POST":
        ...
        return redirect(
            "contact"
        )  # Replace 'contact' with the name of your URL pattern
...
def predict_charges(request):
    ...
    if request.method == "GET":
        return render(
            request, "insurance_app/insurance_form.html", {"prediction": prediction}
        )
...
def book_appointment(request):
    ...
    return render(
        request,
        "insurance_app/book_appointment.html",
        {
            "today": today,
            "form": form,
...
class PredictChargesView(LoginRequiredMixin, UpdateView):
    ...
    def preprocess_data(self, data):
        ...
        df = pd.get_dummies(
            df, columns=["age_category", "bmi_category", "children_str"], dtype=(int)
        )
    ...
    def load_model(self):
        try:
            ...
            print(
                "Error: The file could not be unpickled. Ensure it is a valid pickle file."
            )
            return None
